...
from .clients import StockClient, MarketClient
...
def Get_CWI_History(start_date = '1395-01-01', end_date='1400-12-29', ignore_date = False, just_adj_close = False, show_weekday = False, double_date = False):
    ...
    df_sector['Weekday']=df_sector['Date'].dt.weekday
    df_sector['Weekday'] = df_sector['Weekday'].apply(lambda x: calendar.day_name[x])
    df_sector = df_sector.set_index('J-Date')
    df_sector = df_sector[['Date','Weekday','Open','High','Low','Close','Volume']]
    df_sector[['Open','High','Low','Close','Volume']] = df_sector[['Open','High','Low','Close','Volume']].apply(pd.to_numeric, axis=1)
    if(not show_weekday):
        df_sector.drop(columns=['Weekday'],inplace=True)
    if(not double_date):
        df_sector.drop(columns=['Date'],inplace=True)
    if(not ignore_date):
        df_sector = df_sector[start_date:end_date]
    return df_sector
...
def Get_ShareHoldersInfo(ticker = 'خودرو'):
    ...
    # استخراج WebID
    webid = None
    if hasattr(webid_df, 'iloc'):
        try:
            webid = webid_df.iloc[0]['WebID']
        except Exception:
            webid = webid_df['WebID'] if 'WebID' in webid_df else None
    if not webid:
        print('WebID یافت نشد!')
        return False
    # دریافت داده سهامداران از API جدید TSETMC
    url = f"http://cdn.tsetmc.com/api/Shareholder/GetShareholderList/{webid}"
    headers = {'User-Agent': 'Mozilla/5.0'}
    r = requests.get(url, headers=headers)
    if r.status_code != 200:
        print('دریافت اطلاعات سهامداران ناموفق بود!')
        return False
    data = r.json().get('shareholder', [])
    if not data:
        print('اطلاعات سهامداران یافت نشد!')
        return False
    ...
    for _, row in df.iterrows():
        holder = ShareholdersInfo(
            symbol=ticker,
            date=today,
            holder_name=row.get('shareholderName'),
            holder_type=row.get('shareholderType'),
            shares=row.get('shareholderShares'),
            percent=row.get('shareholderPercent'),
            change=row.get('shareholderChange'),
    ...
    session.commit()
    session.close()
    print(f"{count} رکورد سهامدار برای نماد '{ticker}' ذخیره شد.")
    return df
    # get isin id
